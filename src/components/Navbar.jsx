...
function Navbar() {
    ...
    return (
        <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${Scrolled ? "bg-slate-900/95 backdrop-blur-md shadow-lg py-3" : "bg-transparent py-6"}`}>
            ...
            {/* Menu pour mobile */}
            <div className={`md:hidden absolute top-full left-0 right-0 bg-slate-900/40 backdrop-blur-xl transition-all duration-500 overflow-hidden ${
                menuOpen ? 'max-h-96 border-t border-slate-700/50' : 'max-h-0'}`}>
                {navItems.map((item) => (
                    <button
                        key={item.id}
                        onClick={() => scrollTo(item.id)}
                        className="block w-full text-left px-6 py-4 text-gray-300 hover:text-rose-400 hover:bg-slate-800/30 transition-all">
                        {item.label}
                    </button>
                ))}
            </div>
